...
InsideCarState::InsideCarState(StateStack &stack, Context context)
    : State(stack, context), mPhoneSystem(context) // <--- Pass Context
      ,
      mMaskSelector(context) // <--- Pass Context
{
  std::cout << "[Stack] Entered INSIDE CAR State" << std::endl;

  sf::RenderWindow &window = *getContext().window;
  mBackground.setSize(sf::Vector2f(window.getSize()));
  mBackground.setFillColor(sf::Color(20, 20, 20)); // Dark Grey Interior

  // Placeholder instruction text
  mInfoText.setFont(context.assets->get(GameID::Font::Main));
  mInfoText.setString(
      "1. Wait for Call  2. Select Mask  3. Press ENTER to Drive");
